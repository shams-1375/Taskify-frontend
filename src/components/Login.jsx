import { EyeOff, LogIn, Eye, Lock, Mail } from 'lucide-react'
...
const Login = ({ onSwitchMode, onSubmit }) => {
  ...
  const url = 'https://taskify-backend-1-mrva.onrender.com/api'
  ...
  return (
    <div className=' max-w-md  bg-white w-full shadow-lg border  border-purple-100 rounded-xl p-8'>
      ...
      <div className=' mb-6 text-center'>
        <div className=' w-16 h-16 bg-linear-to-br  from-teal-500  to-cyan-600 rounded-full mx-auto flex items-center justify-center mb-4'>
      ...
      <form onSubmit={handleSubmit} className='space-y-4'>
        {fields.map(({ name, type, placeholder, icon: Icon, isPassword }) => (
          <div key={name} className={INPUTWRAPPER}>
            ...
            {isPassword && (
              <button type='button' onClick={() => setShowPassword((prev) => !prev)} className='ml-2 text-gray-500 hover:text-cyan-500 transition-colors'> {showPassword ? <EyeOff className='w-5 h-5 ' /> : <Eye className='w-5 h-5' />}
              </button>
            )}
          </div>
        ))}
